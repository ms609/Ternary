...
# Interpolation within sampled region
...
# Compute convex hull of our points:
txy <- t(xy)
hull <- chull(txy)
inHull <- if (requireNamespace("sp", quietly = TRUE)) {
  sp::point.in.polygon(tri["x", ], tri["y", ],
                                 xy["x", hull], xy["y", hull])
} else {
  # If package sp is not available, we can use the geometry package
  geometry::inhulln(geometry::convhulln(txy), t(tri[c("x", "y"), ]))
}

# Now we interpolate between our known values to generate a colour for each
# of our tiles
predicted <- ifelse(inHull, Predict(tri[1:2, ]), NA)
#predicted <- Predict(tri[1:2, ])
map <- rbind(x = tri['x', ], y = tri['y', ], z = predicted,
             down = tri['triDown', ])
