...
TreeTools::SpectrumLegend(
  x0 = 0.0, y0 = 0.7, # Position on plot; bottom left = 0, 0
  cex = 0.8, # Font size
  palette = mySpectrum,
  legend = paste(
    seq(from = min(dat$reflectance), to = max(dat$reflectance),
        length.out = 5),
    "%"
  ),
...
if (requireNamespace("TreeTools", quietly = TRUE)) {
  ...
  TreeTools::SpectrumLegend(
    x0 = 0.9,
    y0 = 0.6,
    legend = signif(seq(min(values), max(values), length.out = 4), 3),
