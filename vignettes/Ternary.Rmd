...
```
<!--} else {
  # Discrete legend for point size
  legend(
    "topleft",
    cex = 0.8, # Font size
    pch = 15, # Filled square
    col = rev(viridisLite::viridis(5)),
    legend = paste(
      seq(from = max(dat$reflectance), to = min(dat$reflectance),
          length.out = 5),
      "%"
  ...
  legend(
    "topright",
    pt.cex = seq(maxSize, 0, length.out = 5),
...
```
<!--} else {
  # Discrete legend for colour scale
  legend(
    "topleft",
    legend = round(seq(zRange[1], zRange[2], length.out = 4), 3),
    col = viridisLite::viridis(4L, alpha = 0.6),
    pch = 15,     # Filled square
    bty = "n",    # No framing box
    inset = 0.02,
    xpd = NA      # Do not clip at edge of figure
